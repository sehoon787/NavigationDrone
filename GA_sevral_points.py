    ...
    for i in range(n_generations):
        ...
        for j in range(1, n_cities):
            ...
            if j == n_cities-1:
                cv2.line(
                    map_result,
                    pt1=(fittest[j].x, fittest[j].y),
                    pt2=(fittest[0].x, fittest[0].y),
                    color=(255, 0, 0),
                    thickness=3,
                    lineType=cv2.LINE_AA
                )
        ...
        cv2.putText(map_result, org=(10, 25), text='Generation: %d' % (i + 1), fontFace=cv2.FONT_HERSHEY_SIMPLEX,
                    fontScale=1, color=0, thickness=2, lineType=cv2.LINE_AA)
        cv2.putText(map_result, org=(10, 60), text='Cost : %.2f' % fittest.getDistance(),
                    fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, color=0, thickness=2, lineType=cv2.LINE_AA)
