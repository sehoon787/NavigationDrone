...
# Thread 4
def get_front_img(sock):
    try:
        while True:
            # stringData size from client (==(str(len(stringData))).encode().ljust(16))
            length = recvall(connectionSocket3, 16)
            stringData = recvall(connectionSocket3, int(length))
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        # Front Video Server
                        Front_video_server_IP = "116.89.189.55"
                        Front_video_server_PORT = 22047
                        serverSocket3 = socket(AF_INET, SOCK_STREAM)
                        serverSocket3.bind((Front_video_server_IP, Front_video_server_PORT))
                        serverSocket3.listen(1)
                        print("Waiting Drone Client")
                        connectionSocket3, addr = serverSocket3.accept()
                        print(str(addr), "has connected.")
                        ...
                        # 전면 카메라 쓰레드, 22047
                        get_front = threading.Thread(target=get_front_img, args=(connectionSocket3,))
                        ...
                        print(e)
                        connectionSocket3.close()  # close server
                        serverSocket3.close()
                except Exception as e:
                    print(e)
